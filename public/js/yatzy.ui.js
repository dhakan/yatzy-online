...
$(function() {
  ...
  function addPlayerNameToLobbyList(name, highlight) {
    var $li = $("<li>").text(name);
    if (highlight) {
  ...
  yatzy.websocket.onPlayersConnected(function(players) {
    for (var i = 0; i < players.length; i++) {
      addPlayerNameToLobbyList(players[i].name);
    }
  });

  yatzy.websocket.onPlayerConnected(function(player) {
    addPlayerNameToLobbyList(player.name);
  ...
  yatzy.websocket.onPlayerDisconnected(function(player) {
    ...
    if (player.wasInGame) {
      dicePanel.removeClass("visible").addClass("hidden");
      playerForm.show();
  ...
  yatzy.websocket.onPlayGameWithYourRoundFirst(function(data) {
    ...
    logNewMessage(
      "A game has been initiated and you are playing the first round"
    );
  });
  ...
  function prepareOnlineGame(data) {
    ...
    playerForm.hide();
    dicePanel.addClass("visible").removeClass("hidden");
    $(".scoreBoardCells").remove();
  ...
  yatzy.websocket.onPlayerNameAlreadyChosenByYourself(function(playerName) {
    logNewMessage(
      "You have already submitted this name(" +
        playerName +
        "), alter it and resubmit if you wish to change it!"
    );
  });

  yatzy.websocket.onPlayerNameAlreadyChosenByOtherPlayer(function(playerName) {
    logNewMessage(
      "Your submitted player name(" +
        playerName +
        ") has already been chosen by an opponent, please choose another one!"
    );
  });

  function updatePlayerNameInLobbyList(player) {
    $players.children().each(function() {
      var $player = $(this);
  ...
  yatzy.websocket.onPlayerChangedName(function(player) {
    updatePlayerNameInLobbyList(player);
  ...
  yatzy.websocket.onPlayerNameSubmittedSuccessfully(function(player) {
    if (submittedPlayerName === undefined) {
      logNewMessage("Your name was submitted successfully");
      addPlayerNameToLobbyList(player.name, true);
    } else {
  ...
  $submitPlayerNameBtn.click(function() {
    ...
    if (playerName.trim().length === 0) {
      logNewMessage("The name field cannot be empty or contain only spaces!");
    } else {
      yatzy.websocket.submitPlayerName(playerName.trim());
    }
  });
  ...
  function scoreBoardCellHover() {
    if (
      ...
      $(this).hasClass("activeCellInColumn")
    ) {
      var rowId = $(this)
        .siblings(":first-child")
        .text();
  ...
  function scoreBoardCellClick() {
    ...
    if (
      !$clickedScoreboardCell.hasClass("clicked") &&
      ...
      if (numberOfPlayerScoreboardCellsFilled === 15) {
        ...
        $("#totalRow :nth-child(" + (playerYatzyColumn + 2) + ")").text(
          playerFinalScore
        );
      ...
      yatzy.websocket.saveScoreboardCellValues(
        scoreboardCellsToSendToOtherPlayers
      );
  ...
  function assembleWinnersText(players, numberOfPlayersInvolvedInTie) {
    ...
    if (numberOfPlayersInvolvedInTie === undefined) {
      ...
      for (var i = 0; i < numberOfPlayersInvolvedInTie; i++) {
        winnersText +=
          players[i].name + (i < numberOfPlayersInvolvedInTie - 1 ? ", " : "");
      }
  ...
  yatzy.websocket.onGameComplete(function(data) {
    ...
    var winnersText = assembleWinnersText(
      data.players,
      data.numberOfPlayersInvolvedInTie
    );
  ...
  yatzy.websocket.onGameAlreadyPlaying(function(roundsLeft) {
    logNewMessage(
      "A game is currently running with " + roundsLeft + " rounds left to play"
    );
  });
  ...
  function changePlayerColumnActiveState(inactivate) {
    $("#scoreTable tr :nth-child(" + (playerYatzyColumn + 2) + ")").each(
      function() {
        if (
          !$(this)
            .parent()
  ...
  function getPlayerFinalScore() {
    ...
    $(".totalScore :nth-child(" + (playerYatzyColumn + 2) + ")").each(
      function() {
        var $scoreboardCell = $(this),
          scoreboardCellScore = $scoreboardCell.text();

        playerTotalScore +=
          scoreboardCellScore === "/" ? 0 : Number(scoreboardCellScore);
      }
    );
